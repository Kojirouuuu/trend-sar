...
FILE *fpdata;
//初期採用者はグラフGのみから選ばれる。
double s[Z+1][Z+1][Z+1],ds[Z+1][Z+1][Z+1];//自身の次数がkかつグラフGに属し、qこの外部ノードを持ち、mこの外部隣接iノードとnこの内部隣接iノードを持つsノードとその変化量
double i[Z+1][Z+1][Z+1],di[Z+1][Z+1][Z+1];//自身の次数がkかつグラフGに属し、qこの外部ノードを持ち、mこの外部隣接iノードとnこの内部隣接iノードを持つiノードとその変化量
double s_bar[Z_BAR+1][Z_BAR+1][Z_BAR+1],ds_bar[Z_BAR+1][Z_BAR+1][Z_BAR+1];//自身の次数がkかつグラフG_barに属し、qこの外部ノードを持ち、mこの外部隣接iノードとnこの内部隣接iノードを持つsノードとその変化量
double i_bar[Z_BAR+1][Z_BAR+1][Z_BAR+1],di_bar[Z_BAR+1][Z_BAR+1][Z_BAR+1];//自身の次数がkかつグラフG_barに属し、qこの外部ノードを持ち、mこの外部隣接iノードとnこの内部隣接iノードを持つiノードとその変化量
double S, I;
...
int main(void){
    ...
    sprintf(z_str, "ame/sis/z=%d_z_bar=%d", Z, Z_BAR);

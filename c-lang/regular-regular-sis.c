...
#ifdef _OPENMP
#pragma omp threadprivate(s, ds, i, di, s_bar, ds_bar, i_bar, di_bar, S, I)
#endif
...
/* 平衡値 I と反復回数 itr を返す（出力は呼び出し元の critical で行う） */
typedef struct { double I; int itr; } EvolveResult;

static EvolveResult evolve(double lambda, double mu0, double c, double rho, int k, int k_bar) {
    int itr = 0;
    init(k, k_bar, rho);
    I = rho;
    double I_prev = rho;
    ...
    } while (itr < 1e9 || fabs(I - I_prev) > 0.00000001);
    return (EvolveResult){ .I = I, .itr = itr };
}
...
int main(void) {
    ...
    const double c_min = 2.0;
    const double c_max = 2.0;
    const double c_step = 0.03;
    const double rho = 1.0;

    int total_tasks = 0;
    int length_lambda = 0;
    for (double l = lambda_min; l <= lambda_max + 1e-12; l += lambda_step)
        length_lambda++;
    int length_c = 0;
    for (double cval = c_min; cval <= c_max + 1e-12; cval += c_step)
        length_c++;
    total_tasks = length_lambda * length_c;
...
#ifdef _OPENMP
    fprintf(stderr, "threads: %d | tasks: %d\n", omp_get_max_threads(), total_tasks);
#endif
    int current_task = 0;
    fprintf(stderr, "\r[");
...
#pragma omp parallel for schedule(dynamic)
    ...
    for (int task = 0; task < total_tasks; task++) {
        int i_lam = task / length_c;
        int i_c = task % length_c;
        double lambda = lambda_min + (double)i_lam * lambda_step;
        double c_val = c_min + (double)i_c * c_step;

        EvolveResult r = evolve(lambda, mu0, c_val, rho, Z, Z_BAR);
...
#pragma omp critical
        ...
        {
            if (is_final)
                fprintf(fpdata, "%d,%.6f, %.6f, %.6f, %.6f, %.6f\n", n_r, lambda, mu0, c_val, rho, r.I);
            else {
                double curtime = (double)r.itr * dt;
                fprintf(fpdata, "%d,%.6f, %.6f, %.6f, %.6f, %.6f, %.6f\n", n_r, curtime, lambda, mu0, c_val, rho, r.I);
            }
            current_task++;
            if (current_task % 10 == 0 || current_task == total_tasks) {
                fprintf(stderr, "\r[");
                int filled = BAR_WIDTH * current_task / total_tasks;
                for (int p = 0; p < BAR_WIDTH; p++)
                    fputc(p < filled ? '#' : '-', stderr);
                fprintf(stderr, "] %d/%d (%.1f%%)   ", current_task, total_tasks, 100.0 * current_task / total_tasks);
            }
        }
    }
